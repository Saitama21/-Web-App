...
export function pickBestPriceFact(facts=[],fallbackCurrentPrice=null){
  const normalized=(Array.isArray(facts)?facts:[]).map((fact,index)=>{
    ...
    const confidence=Number.isFinite(Number(fact?.confidence))?Math.max(0,Math.min(1,Number(fact.confidence))):0;
    ...
    if(structured)score+=145;
    score+=Math.round(confidence*30);
    if(fact?.oldVeto)score-=220;
    ...
    return {...checked,...fact,status:structured?'STRUCTURED':checked.status,verified:structured||checked.verified,reason:structured?'Текущая цена подтверждена структурированными данными карточки товара':checked.reason,source,confidence,authoritative,score,index};
